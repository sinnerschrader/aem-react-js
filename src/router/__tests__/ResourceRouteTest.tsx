...
describe('ResourceRoute', () => {
  ...
  class RouterComponent extends ResourceComponent<any, any, any> {
    public renderBody(): React.ReactElement<any> {
      let route = React.createElement(
        Route,
        {
          component: ResourceRoute,
          path: '/index.html',
          resourceComponent: LinkComponent
        } as RouteProps
      );
  ...
  aemTest.init();
  aemTest.currentAemContext.container.register(
    'resourceMapping',
    new ResourceMappingImpl()
  );
  ...
  it('should render content under /index', () => {
    ...
    expect(wrapper.html()).to.equal(
      '<div><div class="dialog"><span><a href="/bla.html"></a></span></div></div>'
    );
  });
  ...
  it('should render loading for unavailable content', () => {
    ...
    expect(wrapper.html()).to.equal(
      '<div><div class="dialog"><span>Loading</span></div></div>'
    );
  });
});
